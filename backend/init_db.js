...
async function init() {
  try {
    console.log('🚀 Initializing EcoCampus Database...');
    ...
    // 1. Departments table (Academic departments/locations)
    await db.query(`
    ...
    // 2b. Login verification table for 2FA login
    await db.query(`
    ...
    // 3. Users table with proper roles and department association
    await db.query(`
      CREATE TABLE users (
        id SERIAL PRIMARY KEY,
    ...
    // 4. Waste logs table with department tracking
    await db.query(`
    ...
    // 5. Tasks table for admin-to-staff assignments
    await db.query(`
      CREATE TABLE tasks (
        id SERIAL PRIMARY KEY,
    ...
    console.log('🎯 Ready for user registration and waste management');
